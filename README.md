...
- **🍽️ "What should I eat?"**: tap the button on the menu, or ask out loud — a free OpenRouter LLM picks **one** recommended dish from today's Counter/Culinarium options (not the full menu list — this is a single pick, not a browse), prioritizing meat/chicken when available. Comes as **two messages**: a quick text line naming which Counter (1-4) or Culinarium section it's at, then the actual pick and reasoning as a real **spoken voice note** (same free TTS pipeline as 🔊 Listen; falls back to text if that's ever unavailable). Always excludes pork, alcohol, wine, and gelatine on top of whatever's in your **🚫 Food Preferences**, and never a weather-based suggestion — purely about the food itself.
