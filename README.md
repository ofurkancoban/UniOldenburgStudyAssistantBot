...
- **📸 Photos for the full menu too**: every time the menu is shown (not just the single "What should I eat?" pick) — `/menu`, tapping through days, or asking out loud — a photo album follows for every dish that could be matched on the Studierendenwerk's public Speiseplan site (split across multiple albums if there are more than 10). Best-effort: if no photos are reachable for that day, the text menu still goes out as normal, just without photos.
