...
- **📸 Every dish, its own message**: opening the menu (`/menu` or asking out loud — not the ⬅️/➡️ day-navigation taps, which stay a single compact text message to avoid a wall of messages on every click) sends each dish as its own message: name, which Counter/Culinarium it's at, description, allergens, and price, with a photo attached when one can be matched on the Studierendenwerk's public Speiseplan site (plain text otherwise) — so it's always obvious which photo belongs to which dish. Best-effort throughout: a fetch/match failure just falls back to text for that dish, never a hard error.
